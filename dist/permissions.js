...
function instances(permission, token) {
  ...
  if (match(p.app, permission.app) && match(p.type, permission.type) && match(p.action, permission.action)) {
...
function userInstances(user, token) {
  ...
  if (user.roles && user.roles.length) {
    ...
    if (_lodash2['default'].isString(user.roles[0])) {
...
function permit(permission, token) {
  var p = parse(token);
  permission = parse(permission);
  return match(p.app, permission.app) && match(p.type, permission.type) && match(p.action, permission.action) && match(p.instance, permission.instance);
}
...
function userPermit(user, token) {
  ...
  if (!user) {
    return _errors2['default'].reject('You do not have permission to perform this action.', 403, 'PERMISSION_DENIED');
  }

  if ((user.roles || []).some(function (r) {
    return rolePermit(r, token);
  }) || (user.permissions || []).some(function (p) {
    return permit(p, token);
  })) {
...
function userAction(user, permission, action) {
  return userPermit(user, permission).then(function () {
    return action ? (0, _q2['default'])(action(userInstances(user, permission))) : (0, _q2['default'])(userInstances(user, permission));
  });
}
